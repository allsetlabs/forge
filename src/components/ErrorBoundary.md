...
> Renders children normally. If an error occurs, shows full-screen error UI with centered card, error icon, message, and action buttons
...
> Centered card with backdrop blur (`flex min-h-screen items-center justify-center`), error icon in red circular background (48px), "Something went wrong" title (3xl, bold), apologetic message
...
- Go Home button navigates via `window.location.href = getHomeUrl()`
- `getHomeUrl()` detects GitHub Pages deployments: if `window.location.hostname` includes `github.io`, it extracts the first path segment as the base path (e.g., `/portfolio/` from `subbiah2806.github.io/portfolio/chat`); otherwise falls back to `/`
