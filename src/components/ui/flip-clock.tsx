...
const FlipUnit: FC<FlipUnitProps> = memo(function FlipUnit({
  ...
  return (
    <div className={cn(flipUnitVariants({ size, variant }), className)}>
      ...
      <div className="bg-background/50 absolute left-0 top-1/2 z-30 h-px w-full -translate-y-1/2" />
    </div>
  );
});
...
function DigitSpan({ children, position }: DigitSpanProps) {
  return (
    <span
      className={cn(
        'absolute left-0 right-0 flex w-full items-center justify-center',
...
function ClockSeparator({ size }: { size?: FlipClockSize }) {
  return (
    <span className={cn('-translate-y-[8%] text-center', size ? heightMap[size] : heightMap['md'])}>
      :
    </span>
  );
}
