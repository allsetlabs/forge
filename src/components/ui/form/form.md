...
Hook to access the current form field's state within a `FormField` context. Returns field metadata and validation state.

```tsx
const { id, name, formItemId, formDescriptionId, formMessageId, invalid, error } = useFormField();
```
...
| `error`             | `object`  | Field error object from React Hook Form  |

> Must be used inside a `<FormField>` component. Throws an error otherwise.
