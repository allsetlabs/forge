...
| `defaultMuted` | `boolean`   | `false` | When true, initializes audio as muted. User can still toggle on. Overridden by localStorage. |
...
2. If no preference, defaults to `defaultMuted` prop value (false if not specified)
...
- Audio element is created with `loop = true` and `volume = 0.3` for background audio playback
- Default state is **unmuted** unless `defaultMuted` is set to true (user must opt-out)
...
- Click listener uses default event listener options
