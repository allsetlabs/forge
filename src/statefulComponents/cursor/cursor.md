...
| `targetElement` | `HTMLElement` | **Required**. The target element where cursor styles should be applied |
...
- Supports Shadow DOM: CSS is injected into the target element's root
- Cursor only tracks and displays within the target element bounds
- Supports `data-panel-resize-handle-id` and `data-clickable` attributes for hover detection
- Dragging state provides visual feedback for resize handles
