...
export function CursorProvider({ children, targetElement }: CursorProviderProps) {
  ...
  // Check device capabilities (only meaningful after mount)
  const canUseCursor =
    mounted &&
  ...
  useEffect(() => {
    ...
    const deviceSupports =
      window.matchMedia('(pointer: fine)').matches &&
      !window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    setIsEnabled(stored === null ? deviceSupports : stored === 'true');
  }, []);
          ...
          <div
            ...
            style={{
              ...
              transition: isDragging
                ? 'none'
                : 'border-color 100ms, background-color 100ms, transform 100ms, box-shadow 100ms',
            }}
            aria-hidden="true"
          />
          {/* Inner dot */}
